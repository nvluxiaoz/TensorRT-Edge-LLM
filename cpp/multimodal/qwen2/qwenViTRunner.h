...
struct QwenViTConfig
{
    ...
    int64_t maxNumImages{0};           //!< Max cu_seqlens entries (images / video temporal groups) per request.
...
protected:
    ...
    virtual std::tuple<int64_t, int64_t> getResizedImageSize(
        int64_t numFrames, bool isVideo, int64_t height, int64_t width, int64_t maxRatio = 200);
    ...
    //! \param[in] spansPerRequest Per-request span counts; a pad may only consume its own request's spans
    ...
    //! \param[in] spansPerRequest Per-request span counts (empty for the media-free system-prompt path)
    ...
    //! \param[out] spansPerRequest Number of spans each request contributed (in request order)
    //! \param[in] stream CUDA stream for execution
    //! \throws std::runtime_error if aspect ratio is invalid
