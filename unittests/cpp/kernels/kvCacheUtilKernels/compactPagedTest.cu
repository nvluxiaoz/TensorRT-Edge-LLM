...
// Final eviction (every request finished, newBatch == 0): the mapping is all -1, nothing survives,
// and compactBatch must be a cheap no-op that leaves every row byte-identical — this is the
// guaranteed end-of-generation path, so it must not scale with the allocated capacity either.
TEST(CompactPagedTest, AllEvictedNewBatchZeroLeavesPoolUntouched)
{
    cudaStream_t stream{nullptr};
    ...
    auto mapping = uploadMapping({-1, -1, -1});
    ASSERT_NO_THROW(mgr.compactBatch(mapping, oldBatch, /*newBatch=*/0, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));

    // No row moved: every old slot still holds its own values in both halves.
    for (int32_t L = 0; L < numLayers; ++L)
    {
        auto [k, v] = mgr.getSeparateKVCache(L);
        for (int32_t b = 0; b < oldBatch; ++b)
        {
            expectRowRangeEq(k, b, 0, maxSeq, kVal(L, b), "K L=" + std::to_string(L));
            expectRowRangeEq(v, b, 0, maxSeq, vVal(L, b), "V L=" + std::to_string(L));
        }
    }
}
