...
{
    return gdnTreeChunkBufferBytes(h, hv);
}
...
class GdnLinearCausalMaskTest : public ::testing::TestWithParam<int32_t>
{
};

TEST_P(GdnLinearCausalMaskTest, MatchesDegenerateTree)
{
    int32_t const numNodes = GetParam();
    constexpr int32_t batch{2};
    size_t const maskCount = static_cast<size_t>(batch) * numNodes * static_cast<size_t>(kGDN_TREE_CHUNK_MASK_WORDS);
    uint32_t* masksDev = nullptr;
    ASSERT_EQ(cudaMalloc(&masksDev, maskCount * sizeof(uint32_t)), cudaSuccess);

    ASSERT_EQ(gdnLinearBuildCausalMasks(masksDev, batch, numNodes, nullptr), cudaSuccess);
    ASSERT_EQ(cudaDeviceSynchronize(), cudaSuccess);
    auto const masks = downloadVec(masksDev, maskCount);

    for (int32_t batchIdx = 0; batchIdx < batch; ++batchIdx)
    {
        for (int32_t node = 0; node < numNodes; ++node)
        {
            for (int32_t word = 0; word < kGDN_TREE_CHUNK_MASK_WORDS; ++word)
            {
                uint32_t expected{};
                for (int32_t ancestor = 0; ancestor <= node; ++ancestor)
                {
                    if (ancestor / 32 == word)
                    {
                        expected |= 1u << (ancestor % 32);
                    }
                }
                size_t const offset
                    = (static_cast<size_t>(batchIdx) * numNodes + node) * kGDN_TREE_CHUNK_MASK_WORDS + word;
                EXPECT_EQ(masks[offset], expected) << "batch=" << batchIdx << " node=" << node << " word=" << word;
            }
        }
    }

    EXPECT_EQ(cudaFree(masksDev), cudaSuccess);
}

INSTANTIATE_TEST_SUITE_P(VerifyLengths, GdnLinearCausalMaskTest, ::testing::Values(4, 5, 6, 7));
